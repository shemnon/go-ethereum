...
func useShortForm(code []byte, interpreter *EVMInterpreter) bool {
	if interpreter.evm.chainRules.IsPrague || interpreter.evm.chainRules.IsOsaka {
...
func opExtCodeCopyEIP7702(pc *uint64, interpreter *EVMInterpreter, scope *ScopeContext) ([]byte, error) {
	...
	if useShortForm(code, interpreter) {
		code = code[:2]
	}
...
func opExtCodeHashEIP7702(pc *uint64, interpreter *EVMInterpreter, scope *ScopeContext) ([]byte, error) {
	...
	if useShortForm(code, interpreter) {
		...
		slot.SetBytes(crypto.Keccak256(code[:2]))
	} else {
