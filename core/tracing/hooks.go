...
type (
	...
	*/
	...
	OpcodeHook = func(pc uint64, section uint64, op byte, gas, cost uint64, scope OpContext, rData []byte, depth int, functionDepth uint64, err error)

	// FaultHook is invoked when an error occurs during the execution of an opcode.
	FaultHook = func(pc uint64, section uint64, op byte, gas, cost uint64, scope OpContext, depth int, functionDepth uint64, err error)
