...
func (t *muxTracer) OnOpcode(pc uint64, section uint64, op byte, gas, cost uint64, scope tracing.OpContext, rData []byte, depth int, functiondepth uint64, err error) {
...
func (t *muxTracer) OnFault(pc uint64, section uint64, op byte, gas, cost uint64, scope tracing.OpContext, depth int, functiondepth uint64, err error) {
	for _, t := range t.tracers {
		if t.OnFault != nil {
